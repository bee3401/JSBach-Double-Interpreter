...
class EvalVisitor(jsbachVisitor):
    def __init__(self, start_function, parameters):
        self.variables          = {}                                # diccionari de de diccionaris: guarda, per a cada mètode, les seves variables 
        self.local_variables    = {}                                # diccionari amb les variables del mètode actual
        self.methods            = {}                                # diccionari amb la informació necessària de tots els mètodes. El nom, paràmetres i punter d'accés
        self.parameters         = parameters                        # possibles paràmetres d'entrada
        self.arrays             = {}                                # diccionari amb totes les llistes. Clau d'accés == direcció en memòria (see readMe)
        self.current_function = start_function                      # funció inicial, pot ser main o una altra
        self.array            = []                                  # llistat auxiliar per llegir i emmagatzemar llistes 
        self.function_stack   = []                                  # stcak amb totes les funcions
        self.read_only        = True                                # permet saber si estem fent una primera lectura de les funcions per recopilar informació, o si estem visitant les funcions en temps d'execució
        self.keys             = ["A", "B", "C", "D", "E", "F", "G"] # Notes
        self.sheet            = []                                  # Estructura amb les notes que es reprodueixen al final de l'execució
        self.memory_sim       = {}                                  # simulador de direccions de memòria; diccionari amb el nom de les llistes, i a quina direcció de self.arrays apunten
        self.first_av_address = 0                                   # primera direcció de memòria disponible
    ...
    def visitRoot(self, ctx):
        ...
        # En cas que la primera funció tingui paràmetres, els guardem, ja que venen llegits del canal d'entrada estàndard
    ...
    def visitFunction(self, ctx):
        ...
        if not self.read_only:
            ...
            #returns fuction name and parameters
            parameters = []
            i = 1
            while l[i].getText() != '|:':
    ...
    def visitExpr(self, ctx):
        ...
        if len(l) == 1:
            ...
            if l[0].getText() == '(':
                ...
                b = self.visit(l[2])
                op = l[i+1].getText()
                if op in numeric_op_dictionary:
                    if op == '/' and int(b) == 0:
                        raise Exception("Error: Expr tried to divide by zero")
                    else:
                        r = numeric_op_dictionary[op] (int(a), int(b))
                        return int(r)
                elif logical_op_dictionary[op] (int(a), int(b)) == True:
                    return 1
                else: 
                    return 0
